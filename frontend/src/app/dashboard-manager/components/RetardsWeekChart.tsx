...
interface RetardsWeekData {
  jour: string;
  count: number;
  isToday?: boolean;
}

interface RetardsWeekChartProps {
  data: RetardsWeekData[];
  total: number;
  evolution: number; // Pourcentage d'évolution vs semaine dernière
}

export default function RetardsWeekChart({ data, total, evolution }: RetardsWeekChartProps) {
    ...
    <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-200">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-full flex items-center justify-center" style={{ backgroundColor: 'rgba(51, 51, 51, 0.1)' }}>
            <TrendingDown size={24} style={{ color: '#333333' }} />
          </div>
          <h3 className="text-xl font-bold" style={{ color: '#333333' }}>Retards Semaine</h3>
        </div>
      </div>
      ...
      <div className="mt-8 pt-4 border-t border-gray-200">
        <div className="flex items-center justify-between">
          ...
          <span className="text-lg font-bold" style={{ color: '#333333' }}>{total} retards</span>
        </div>
        {evolution !== 0 && (
